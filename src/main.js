...
// Vite が本番ビルド時に置換する BASE_URL を直接文字列連結
const AUTO_LOAD_URL = (import.meta.env.BASE_URL || './').replace(/\/?$/, '/') + 'samples/rohbau_site02_scan0.ply';
console.info('[autoload] fetching:', AUTO_LOAD_URL);

fetch(AUTO_LOAD_URL)
  .then((res) => {
    if (!res.ok) throw new Error(`HTTP ${res.status} @ ${AUTO_LOAD_URL}`);
    return res.arrayBuffer();
  })
  .then((buf) => handleFiles([new File([buf], 'rohbau_site02_scan0.ply')]))
  .catch((err) => {
